...
import { PlusIcon as Plus, TagIcon as Tag } from '@phosphor-icons/react'
...
export default function CategoriasProductosPage() {
  ...
  async function load() {
    try {
      const res = await categories.list({ limit: 100, type: 'product', active: 'all' })
      setData(res.data)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Categorías para el catálogo de productos"
      ...
      <Drawer
        open={drawer.open}
        onClose={closeDrawer}
        title={drawer.item ? 'Editar categoría' : 'Nueva categoría — Productos'}
      >
        <CategoryForm
          key={drawer.item?._id ?? 'new-product'}
          initial={drawer.item}
          lockedType="product"
