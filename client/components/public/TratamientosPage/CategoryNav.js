  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex gap-3 py-4 md:py-5">
        {[{ _id: "todos", name: "Todos los tratamientos" }, ...categories].map(
          (cat) => {
            ...
            return (
              <button
                ...
                className="shrink-0 flex items-center justify-center px-8 py-3 md:px-10 md:py-4 text-sm font-semibold tracking-widest uppercase transition-all duration-200 cursor-pointer whitespace-nowrap rounded-full active:scale-[0.97]"
                style={{
                ...
                onMouseEnter={(e) => {
                  if (!isActive) {
                    e.currentTarget.style.background = "rgba(61,189,181,0.08)";
                    e.currentTarget.style.borderColor = "var(--pub-accent)";
                  }
                }}
                onMouseLeave={(e) => {
                  if (!isActive) {
